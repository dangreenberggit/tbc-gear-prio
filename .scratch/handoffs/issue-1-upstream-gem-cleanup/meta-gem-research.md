...
No spec surfaced a genuine phase-to-phase static-best-changes (e.g. "P1-P2 use meta X, P3+ switch to meta Y") pattern, and no crit/hit-rating breakpoint language was found anywhere. A static per-spec table (one meta gem per spec) is therefore defensible.

Both provisos originally listed here — (1) Chaotic Skyfire Diamond's TBC-Classic-phase availability, and (2) upstream `presets.ts` confirmation of one static meta per spec — are **CLOSED**, not open. See "Local verification pass" above: (1) `vendor/wowsims/db.json` records `phase: 1` for both 32409 and 34220, so there is no pre-P3 caster gap to hedge on; (2) the vendored ret and feral gear-set presets were read directly (`cat vendor/wowsims/ret_*.gear.json vendor/wowsims/feral_*.gear.json`), confirming ret is static on 32409 across all three phases and feral sockets no meta at all. Discharged 2026-08-12, commit `23df60f`.
