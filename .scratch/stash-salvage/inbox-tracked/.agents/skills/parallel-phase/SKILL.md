---
name: parallel-phase
description: Fan-out parallel independent slices on a feature branch. Use when tasks can run in parallel (different kinds of work, mostly disjoint files), when the user asks to fan out / delegate / orchestrate workers, or when another skill needs worktree-isolated parallel coding. For a new phase-N/* or large feat/*, start with Step 0 (sharp planner → phase execution plan) before partitioning workers.
---
...
Cap concurrency around **3–5** unless a scripted cloud orchestrator is driving the tree. Prefer fewer, broader workers over many tiny ones. Workers use the **workhorse** model lane — do not fan out a swarm of high-ticket sharp models (Opus at effort `high`+, Fable, Sol); see [`docs/agents/model-policy.md`](../../../docs/agents/model-policy.md). On a rate-limit wall, serialise or wait — do not silently drop to a toy model for implementation.
...
0. **Phase / feature plan first (before any fan-out).** Opening a `phase-N/*`
   or large `feat/<slug>` is **not** "partition and spawn." PLAN.md §14 gates
   are the destination; they are too coarse to drive day-to-day slices. Before
   Step 1:

   - Spin a **sharp** planner on **this harness’s** sharp lane (on Cursor:
     Grok high — do not Sol-probe first; see
     [`docs/agents/model-policy.md`](../../../docs/agents/model-policy.md)), not
     a workhorse implementation agent. One planner, not a swarm.
   - Deliverable: a **phase execution plan** on the feature branch under
     `.scratch/handoffs/phase-N-execution-plan.md` (or `feat-<slug>-execution-plan.md`)
     that turns the §14 gate into ordered slices with acceptance tests, known
     blockers, and explicit "green lights" (verify recipes) *ahead of* the work —
     not a traffic-light map written after you hit red.
   - The plan is load-bearing for partition: Step 1's slices should cite it.
     A reactive checklist filed mid-phase is a recovery tool, not a substitute
     for this step.
   - Skip only when the user already supplied an execution plan of equal
     specificity, or explicitly waived planning for a tiny spike.

1. **Partition** — On `feat/<slug>` or `phase-N/<slug>`, list slices **from the
   execution plan**. For each: goal, `pathsAllowed` / `pathsForbidden`,
   acceptance, verify recipe. Minimize path overlap.
