...
WEAPON_POLEARM = 6
WEAPON_STAFF = 8
RANGED_LIBRAR = 7  # proto.RangedWeaponType Libram
# Top-N per slot before curation trims toward ~8 (PLAN.md §8.3.3).
TOP_N = 12
MIN_QUALITY = 3  # rare+
# White-damage proxy weight so real 2H sticks beat hit-stat rares on EP.
WEAPON_DPS_EP = 12.0

# Kael'thas TK legendaries — encounter-only, not persistent upgrades.
KAEL_TEMP_LEGENDARY_IDS = {
...
# db scaling sometimes stores 1H speed on 2H items — distorts white-DPS EP ranking.
BAD_2H_SPEED_FALLBACK = 3.6


def weapon_damage_ep(it: dict) -> float:
    """EP from white damage. Stats-only EP ignores this and ranks hit rares above real 2H."""
    scaling = (it.get("scalingOptions") or {}).get("0") or {}
    wmin = scaling.get("weaponDamageMin")
    wmax = scaling.get("weaponDamageMax")
    speed = it.get("weaponSpeed") or scaling.get("weaponSpeed")
    if wmin is None or wmax is None or not speed:
        return 0.0
    speed = float(speed)
    if it.get("handType") == HAND_TYPE_TWO_HAND and speed < 2.5:
        speed = BAD_2H_SPEED_FALLBACK
    dps = (float(wmin) + float(wmax)) / 2.0 / speed
    return dps * WEAPON_DPS_EP
...
def ret_equippable(it: dict) -> bool:
    if it.get("id") in KAEL_TEMP_LEGENDARY_IDS:
        return False
    t = it.get("type")
    if t is None:
        return False
    ...
    if slot == "weapon":
        # Ret ranks main-hand 2H only; db 1H swords are caster/off-spec noise.
        ...
        return True
    if slot == "ranged":
        # Ret ranged is a libram, not bows/guns/thrown.
        return it.get("rangedWeaponType") == RANGED_LIBRAR
    return True
...
def generate(spec: str) -> dict:
    ...
    for it in db["items"]:
        ...
        stats = item_stats(it)
        score = ep_score(stats, w) + weapon_damage_ep(it)
        if score <= 0:
            continue
        entry = {
            "itemId": it["id"],
