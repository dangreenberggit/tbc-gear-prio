...
# Ticket 171 (user ruling, exclusion by design): an item whose proc/on-use
# effect is only a commented `TODO: Manual implementation required` stub in
# the pinned fork's Go source is dropped from every universe entirely -- not
# simmed, not shown. Generated by generate_sim_implemented_effects.py; see
...
def stub_only_effect_ids() -> dict[int, str]:
    """itemId -> name, for items whose only sim effect is an unimplemented stub.

    Empty (not an error) when data/sim-implemented-effects.json has not been
    generated yet -- this file is committed once produced, but a worktree
    without the fork checked out cannot regenerate it, and assembling a
    universe must not hard-fail on that absence the way the generator itself
    does. See generate_sim_implemented_effects.py.
    """
    if not SIM_IMPLEMENTED_EFFECTS.is_file():
        return {}
    data = load_json(SIM_IMPLEMENTED_EFFECTS)
    assert isinstance(data, dict)
    return {
        int(row["itemId"]): str(row["name"])
        for row in data.get("stubOnlyItemIds") or []
...
def assemble(
    ...
    stub_only_ids = stub_only_effect_ids()
    excluded_unimplemented: list[dict] = []
    ...
    for it in db["items"]:
        ...
        if not pairs:
            no_zone_excluded += 1
            continue
        if iid in stub_only_ids:
            excluded_unimplemented.append(
                {"itemId": iid, "name": it.get("name") or stub_only_ids[iid]}
            )
            continue
    ...
    report = {
        ...
        # Ticket 171 (user ruling): items dropped because their only sim
        # effect is an unimplemented stub in the pinned fork. Keeps recall
        # bookkeeping honest -- these ids are D7-eligible and sourced, so
        # without this list they would look indistinguishable from
        # `excludedNoSource` in a diff of the report.
