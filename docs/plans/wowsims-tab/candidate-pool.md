...
| F2  | Ret eligible counts by `maxPhase`, from `ret-p5.json`: p1 **155**, p2 **246**, p3 **390**, p4 **437**, p5 **518**. Count: `node -e "const u=require('./data/universes/ret-p5.json');for(const p of [1,2,3,4,5])console.log(p,u.filter(r=>r.phase<=p).length)"` (adjust if the file is wrapped in an object).                                                               | `data/universes/ret-p5.json`                                                 |
...
On the **tuning fixture** (ret, slamaltman) run the full eligible set at `maxPhase 2` at each sweep point of 3.1 and at 5,000. For each point report: Spearman rank correlation of screening delta vs 5,000-iteration delta; and **K\*** = the smallest global top-K by screening rank that contains every 5,000-iteration above-cutoff row. Output `experiments/e-w5-rank.{json,md}`.

M2 **go** if some point has `cost(point)/cost(5000) < 0.25` **and** `K* ≤ 60`; that point is `screenIterations`, `promoteTopK = K* + 10`. Otherwise **no-go**: M2 is not shipped, this file says so, and M3 goes to a design pass.
...
Test runners: recorded full-iteration observations stay as they are. Screening observations come from a **derived runner** — recorded full-iteration truth plus deterministic seeded noise scaled by `1/sqrt(iterations)` (F12's `respondingSim` shape) — so `screenIterations` can change without re-recording, and recall can be run over many draws. One real screening recording, pinned, as a smoke test only. **Fixture split:** tune defaults on ret/slamaltman; gate recall on feral (`packages/core/src/fixtures/feral-offline.ts`); the tuning fixture never gates.
