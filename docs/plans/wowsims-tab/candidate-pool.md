...
Promote _c_ if any of: _c_ is in the global top-`promoteTopK` by screening delta; _c_ is best-in-slot at screening (floor: no empty slot); _c_ is in a set-completion package; _c_ is owned. Everything else is **screened out**: keeps screening `deltaDps`, flagged `screened: {iterations, promoted: false}`, is a **third view state** (not below-cutoff: unknown, not known-small), and is returned by `applyView` as its `ruledOut` set — ordered by slot then name, never interleaved with full-iteration deltas and never given a position, because a delta-ordered list still reads as a ranking (ticket 224). It renders behind its own expand (`--show-ruled-out`; a collapsed per-slot block in the HTML report). The screening ordering stays on `Ranking.items`. Hidden, never deleted.
