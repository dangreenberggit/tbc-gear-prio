...
| F10 | `PLAN.md` §10: five-seed max−min of baseline means was **0.099 DPS** at 5,000 iterations — **that 0.099 measures seed overlap, not sim stability** (ticket 236: those five seeds share almost all their RNG streams; see the correction in `docs/verification-log.md`), while the reported-SE figure beside it is unaffected; **reported independent SE 1.678 DPS** at 5,000; cutoff `{absDps: 3.4, pct: 0.15}`. Only independent SE is observable per sim; paired-delta variance is not returned by the sim. | `docs/five-seed-spread.json`, `packages/core/src/se.ts:1-12`                 |
...
> **Superseded by [ADR-0026](../../adr/0026-racing-is-removed-the-engine-full-sweeps-every-eligible-candidate.md) (2026-08-19).** Racing was built as specified below and then removed: measured against a bar fixed before the measurement, it costs _more_ than the full sweep it was meant to avoid (wall ratios 1.407/1.476/1.098 at shipped defaults), and it still misses the 20 % bar at an oracle per-pool K tuned on the answer (0.651/0.899/0.868). No production caller ever opted in. The engine now full-iteration-sims every eligible candidate, and §3.4.1's M2 decision goes with it. This section is kept as the record of what was built and measured, not as a description of the engine. The recall obligation in §7 did not lapse — it moved to `packages/core/test/full-sweep-recall.test.ts`. Adaptive-CI screening, the one alternative that cleared the cost term, is filed as ticket 237 rather than built.
...
Promote _c_ if any of: _c_ is in the global top-`promoteTopK` by screening delta; _c_ is best-in-slot at screening (floor: no empty slot); _c_ is in a set-completion package; _c_ is owned. Everything else is **screened out**: keeps screening `deltaDps`, flagged `screened: {iterations, promoted: false}`, is a **third view state** (not below-cutoff: unknown, not known-small), and is returned by `applyView` as its `ruledOut` set — ordered by slot then name, never interleaved with full-iteration deltas and never given a position, because a delta-ordered list still reads as a ranking (ticket 224). Its **count** always renders — the CLI prints a one-line disclosure of how many rows were set aside whenever there are any, so a reader who never passes the flag still knows they exist; the **listing** renders behind its own expand (`--show-ruled-out`; a collapsed per-slot block in the HTML report). See the comment above `ruledOutDisclosureLine` in `cli.ts` for why the count is unconditional. The screening ordering stays on `Ranking.items`. Hidden, never deleted.
...
- **Fixture split:** the tuning fixture is ret; **every other roster fixture gates** recall (7.2). The tuning fixture never gates. Built in `packages/core/src/fixtures/synthetic-offline.ts` (ticket 204): `RET_SYNTHETIC_ROW` (pre-raid preset, `maxPhase 2`, **38** above-cutoff of 240 pooled) is the tuning fixture; `FERAL_SYNTHETIC_ROW` (same triple, **42** above-cutoff of 246 pooled) is the one that gates 7.2 — both measured by `scripts/record_synthetic_fixtures.mjs` against pinned `wowsimcli` v0.0.101 and asserted in `packages/core/test/synthetic-fixtures.test.ts`. The pre-existing WCL-derived fixtures (`packages/core/src/fixtures/feral-offline.ts`, `slamaltman-offline.ts`) are **kept alongside, not replaced** — their own tests (`feral-preset.test.ts`, `slamaltman-offline.test.ts`) exercise confidence-from-form-uptime, salvation-uptime and talent-capture behaviour that only exists because WCL data is ambiguous; a synthetic fixture has no such ambiguity to measure, so it cannot stand in for those tests without deleting the behaviour they cover.
...
| 7.7       | applyView: screened rows are a third state, returned in `ruledOut` (never in `rows`/`shortlist`/`groups`), ordered by slot then name with no `tieGroupId`, survive filters; `shortlist + belowCutoffCount + ruledOut` reconciles (ticket 224)                                                                      | unit                                | hidden never deleted, no interleaving          |
...
| P3-recall | Recall on the maxPhase 3 pool (ticket 221): same gate as 7.2, measured at 398 eligible instead of 246. Keyed by token, not number, because vitest `-t` is an unanchored regex where `.` matches any character, so any `7.N` name collides with the §7 rows (ticket 230). Select with `racing.test.ts -t P3-recall` | interface                           | recall survives the larger pool                |
...
| D     | M1 port to fork + adapter (`concurrency`, memoryCap, split fallback) + controls; `makeRaidSimRequest` iterations param                       | Sonnet       | nested fork clone, main working tree (§9.1a)                                                                                      | C merged                                                   |
...
| F     | M2 port to fork; 7.10 parity at tip                                                                                                          | Sonnet       | nested fork clone, main working tree (§9.1a)                                                                                      | D, E merged                                                |
...
The fork is not a sibling clone: `vendor/tbc-new-fork` is a **nested git repo** with its own `.git`, gitignored by this repo (`.gitignore:13`), pinned by `data/wowsims-fork.lock.json` (integration branch `feat/upgrades-tab`; `pushed: false`). Isolation for fork slices:

- **One fork writer per round** (D in Round 2, F in Round 3), so each uses the clone's **main working tree** on a branch `w/candidate-pool-<slice>` off `feat/upgrades-tab`. Only if two fork writers ever overlap does the orchestrator add `git -C vendor/tbc-new-fork worktree add ../../.worktrees/fork-<slice> <branch>` (the pattern `orchestration.md` risk 1 used for slice 5).
- **Before spawning D**, the orchestrator makes the clone clean: the 2026-08-15 setup left `tsconfig.json` modified (`allowImportingTsExtensions`, ticket 156) and an untracked `.ew1-scratch/`. Commit the tsconfig fix on `feat/upgrades-tab` with a message pointing at ticket 156, move or delete `.ew1-scratch/`, and record the resulting SHA as D's base. Do not spawn into a dirty tree.
- **The lockfile is this repo's file, owned by the orchestrator.** After D and F merge inside the fork onto `feat/upgrades-tab`, the orchestrator bumps `commit` in `data/wowsims-fork.lock.json` on `feat/candidate-pool` and re-runs 7.10 against that SHA. `pushed` stays `false` — pushing the fork is a separate ask.
- Workers in the fork never touch this repo; workers in this repo never touch the fork. A slice that needs both (there is none by design) is two slices.
