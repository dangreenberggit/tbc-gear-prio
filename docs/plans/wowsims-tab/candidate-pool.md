...
| F10 | `PLAN.md` §10: five-seed max−min of baseline means was **0.099 DPS** at 5,000 iterations — **that 0.099 measures seed overlap, not sim stability** (ticket 232: those five seeds share almost all their RNG streams; see the correction in `docs/verification-log.md`), while the reported-SE figure beside it is unaffected; **reported independent SE 1.678 DPS** at 5,000; cutoff `{absDps: 3.4, pct: 0.15}`. Only independent SE is observable per sim; paired-delta variance is not returned by the sim. | `docs/five-seed-spread.json`, `packages/core/src/se.ts:1-12`                 |
