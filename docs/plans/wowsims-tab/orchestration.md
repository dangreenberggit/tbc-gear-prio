...
| E-W1 needs `make wasm` (Go ≥1.25 + protoc) which may not be installed | Slice 1's handoff records toolchain versions as a preflight; missing tools → ask the user before Phase C, do not let slice 3 improvise around the gate                                              |
