...
| A3      | Adversarial          | fixed (follow-up) | `.scratch/carry-forward/issues/29-job-row-stranded-on-meta-unsolvable.md` — **closed 2026-08-04** on `fix/job-row-and-ticket-30`: one catch around the whole post-create body. A follow-up review of that fix found it mislabelled non-`RankError` failures as `sim-failed`; corrected to a new `internal` kind, both mutation-checked                                                                                             |
...
| S4      | Spec                 | defer             | `.scratch/carry-forward/issues/30-viewoptions-gate-not-pinned-through-applyview.md`. **Re-filed 2026-08-04 as `Blocks: phase-2`** — first filed `Blocks: phase-1`, which was wrong: the box is on the Phase 2 gate, the Phase 1 gate is closed (`docs/verification-log.md`, 2026-07-29, "Phase 1 gate: 10 of 10"), and the real blocker is that `applyView` is unimplemented (a scheduled Phase 2 deliverable), not a missing test |
