...
| A3                     | Adversarial        | defer       | `.scratch/carry-forward/issues/36-relative-cutoff-within-a-filtered-view.md` — the comment no longer overclaims; whether §12 wants a _relative_ cutoff is a spec question, and the obvious reading collides with §2's "no view changes a number"                                                                                                                                                    |
...
| A5                     | Adversarial        | wontfix     | Correct, and it is the point. Ticket 30 specified this altitude precisely because the pure-function level cannot reach "or trigger a sim". A test that cannot fail while `applyView` stays pure is a _guard on purity_: it fails the moment someone gives `applyView` a seam, which is the regression worth preventing. Kept, with the `> 0` guard so it cannot pass on a ranking that never simmed |
