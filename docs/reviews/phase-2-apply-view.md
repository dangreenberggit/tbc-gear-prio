...
Diffed against: `phase-2/trust...phase-2/apply-view` (f99c486 → 48c6e7f, plus the A5 fix commit below)
...
The gate box has two clauses — "does not change `contentHash` **or trigger a
sim**" — and this left the second one open while it was reported closed. Ticket
30 had flagged exactly that half as the harder one ("what the current test
cannot reach at all"). Two further things fell out of fixing it:

- The claim is about the **caller's loop** — one `rankUpgrades`, N re-renders —
  not about `applyView` in isolation. No assertion inside a pure function can
  express it.
- **A sim counter cannot express it either.** A caller that re-ranked identical
  input runs _zero_ extra sims, because the result cache serves them. The
  caching is correct; it makes the sim count blind to this specific regression.
  Counting entries into the engine via `findFights` — deliberately uncached
  (`gear-source.ts:78`), and the engine's first call — is what actually catches
  it.
...
**The token box is genuinely closed** at the altitude ticket 30 named, with a
premise guard and a negative control. The `ViewOptions` box was **half closed**
when this was written — see A5; the "or trigger a sim" clause is now closed by a
caller-loop test counting engine entries rather than sims.

---

## Summary

Nine findings, eight fixed. The one that mattered was found twice
...
**A5 is the one that should change how this branch is read.** The
`ViewOptions` gate box has two clauses and I reported both closed when only one
was. The second — "or trigger a sim" — was the half ticket 30 had singled out
as unreachable, and my test asserted it on a function that cannot run a sim by
construction. Closing it properly took two corrections: move to the caller's
altitude (one rank, N re-renders), and then stop counting sims at all, because
the result cache means a re-ranking caller costs zero extra sims. Counting
engine entries via the uncached `findFights` is what finally catches it —
mutation-checked at `expected 15 to be 1`. A green suite and a closed-looking
gate box are not the same thing, and this branch had both while the box was
open.
...
`pnpm verify` green: 292 tests, 27 files (sha updated on the A5 fix commit).
...
| A5                     | Adversarial        | fixed       | Upheld, and it left the gate box half open. The box has two clauses — "does not change `contentHash` **or trigger a sim**" — and only the first was closed: `applyView(ranking, view)` holds no `SimRunner`, so the run-count assertion was trivially true. Closing the second needs the **caller's** altitude, since the claim is about one `rankUpgrades` serving N re-renders. The first attempt at that was also theatre: a re-ranking caller runs **zero** extra sims, because identical input hits the result cache — so a sim counter cannot see this regression at all. Now counted via `findFights`, which is deliberately uncached (`gear-source.ts:78`) and is the engine's first call. Mutation-checked: a caller re-ranking per view change fails with `expected 15 to be 1` |
