...
| 3-A1  | Adversarial | fixed       | `102b425` — disclosure half fixed: render-time qualifier (`withSelfConfoundDisclosed`) derives the coupling from the sibling 2pc row already in the JSON, HTML regenerated render-only, tested at 1-worn and 0-worn; ticket 127 closed. The arithmetic half stays open as ticket 119 (owner decision), and the JSON still carries the bare number — 127's closing note says so |
| 3-A2  | Adversarial | fixed       | `2f29cf9` — hide rules now outrank the package-only reveal (`:not(.source-hidden)` on the reveal, higher-specificity BiS guard), matching the JS exclusion; selectors pinned by string tests, artifact HTML regenerated render-only; ticket 128 closed                                                                                                                         |
| 3-A3  | Adversarial | fixed       | `75d3f87`/`d114e91` — ticket 114 closed this round (was `defer` in round 3): `gemsForQuality` throws on non-number quality, cap moved into the `fillEligibleGems` chokepoint                                                                                                                                                                                                   |
| 3-D1  | Domain      | defer       | `.scratch/carry-forward/issues/133-negative-plausibility-band-width-is-inherited-not-measured.md`                                                                                                                                                                                                                                                                              |
| 3-D2  | Domain      | fixed       | `102b425` — `GEM_POLICY_QUALIFIER` gained the meta-repair-may-recolour clause; goldens repinned, ticket 107 annotated and stays open for its per-row data work                                                                                                                                                                                                                 |
| 3-D3  | Domain      | defer       | `.scratch/carry-forward/issues/121-no-upstream-ret-p3-curated-gear-set-to-pin.md` — already open; the ADR-0024 reference-gear caveat ask for the +11.31 mana-proc figure is now recorded in the ticket itself                                                                                                                                                                  |
| 3-St1 | Standards   | fixed       | `034cb0e` — `package.json` restored to LF (blob 67→0 CRLF) and pinned via `.gitattributes` `package.json text eol=lf`; ticket 129 closed                                                                                                                                                                                                                                       |
| 3-St2 | Standards   | fixed       | `3935d05` — `gems.ts` docstring now says both fill and meta repair cap rarity; ticket 130 closed                                                                                                                                                                                                                                                                               |
| 3-St3 | Standards   | fixed       | `3935d05` — misplaced `curatedPointer` block removed, `packageLineText` comment now matches the any-non-empty-packages guard per ADR-0024                                                                                                                                                                                                                                      |
| 3-St4 | Standards   | fixed       | `3935d05` — test renamed to the provenance behaviour it actually asserts; assertion kept                                                                                                                                                                                                                                                                                       |
...
| 3-S1  | Spec        | fixed       | `29a2f6c` — ADR-0023 decision 5 narrowed in place: it forbids restating the confounded `bonusDps`, not `packageDeltaDps`, so ADR-0024's pointer is consistent with its intent; ticket 131 closed                                                                                                                                                                               |
...
| 5-D2  | Domain      | fixed       | `7104fb6` — pin test "no sockets ⇒ vacuously active, contributes zero"; ticket 142 closed                                                                                                                                                                                                                                                                                      |
| 5-D3  | Domain      | fixed       | `016b019` — stale Verdict section corrected to match the closed findings; ticket 143 closed                                                                                                                                                                                                                                                                                    |
| 5-D4  | Domain      | fixed       | `7104fb6` — `SPEC_PREFERRED_METAS` comment now says what to do when `DetectedSpecId` widens; ticket 142 closed                                                                                                                                                                                                                                                                 |
| 5-S1  | Spec        | fixed       | `dfe4ae6` (+ `8a4da79` follow-up) — gem-swap disclosure now reaches package rows; ticket 144 closed                                                                                                                                                                                                                                                                            |
...
| P3    | Process     | fixed       | `016b019` + `a1d3ef6` — duplicate renamed to ticket 153, cross-linked with 121; ticket 146 closed                                                                                                                                                                                                                                                                              |
| P4    | Process     | fixed       | `71eb58f` + `1f03344` — statuses normalised, regex widened, unparseable status is now an error; ticket 147 closed                                                                                                                                                                                                                                                              |
| P5    | Process     | fixed       | Observations recorded; `d8b0698` verified to cover the model-inheritance path; ticket 148 closed                                                                                                                                                                                                                                                                               |
| P6    | Process     | defer       | `.scratch/carry-forward/issues/149-six-worktrees-still-registered-after-fan-out.md`                                                                                                                                                                                                                                                                                            |
| 6-A1  | Adversarial | fixed       | `b16e0b1` (+ `8a4da79` follow-up) — test observed red pre-fix; unclassifiable slot now warns instead of vanishing; ticket 150 closed                                                                                                                                                                                                                                           |
| 6-A2  | Adversarial | fixed       | `b16e0b1` — no-ownership fallback replaced by explicit `unidentified-worn-item` refusal; ticket 151 closed                                                                                                                                                                                                                                                                     |
| 6-A3  | Adversarial | fixed       | `f92a59c` — unmeasured guard added, test observed red pre-fix; backfill now matches the live path; ticket 152 closed                                                                                                                                                                                                                                                           |
| 6-A4  | Adversarial | fixed       | `b16e0b1` — `tiedCandidates` now surfaced via `deadSlotWarnings`; ticket 151 closed                                                                                                                                                                                                                                                                                            |
