...
| A3  | Adversarial  | defer       | `.scratch/carry-forward/issues/165-ew3-cannot-fail-on-request-composition.md` — folded in as the `combineSe` invariant to pin; untraced suspicion, not touched by this diff                                                                                            |
...
| D3  | Domain       | fixed       | fork `3000b2f6b` / outer `8229c97` — the tab now resolves the committed weights file per spec **and** phase from the shared `ep-weights-by-phase.json`, so the disclosure cannot name the wrong file after branch B merges (ticket 168 resolved)                       |
...
| S4  | Spec         | fixed       | fork `3000b2f6b` / outer `8229c97` — page EP weights are used by default when customized (`hasCustomEPWeights()` guard + validity rules), else the per-phase committed file; drawer discloses which (ticket 162 v2 resolved, user decision 2026-08-15)                 |
...
| S7  | Standards    | defer       | `.scratch/carry-forward/issues/166-fork-tests-are-invisible-to-pnpm-verify.md` — folded in; it forces the same decision about how fork tests are run                                                                                                                   |
