...
| A2  | Adversarial | defer       | `.scratch/carry-forward/issues/31-sqlitestore-job-ids-and-kv-created-at.md` — `job.create` wedges with `UNIQUE constraint failed` after any row deletion                                                                                                 |
| A3  | Adversarial | defer       | `.scratch/carry-forward/issues/31-sqlitestore-job-ids-and-kv-created-at.md` — a top-level `undefined` input round-trips to `null` on `SqliteStore` but stays `undefined` on `MemoryStore`                                                                |
