# ADR-0016 — The second `GearSource` and `Store` adapters move to Stage 2

**Status:** accepted
**Date:** 2026-07-30
**Supersedes:** the Stage 1 timing commitment in PLAN.md §5 (not the rule itself)
...
> below has two adapters that both actually get built, in Stage 1, and both
> actually get used.

Stage 1 ships three seams. Verify what each actually has with:
...
The consequence is not that anything is broken — every Stage 1 claim about
...
scopes as Stage 2's caching work. Building it inside Stage 1 would pull a
Stage 2 concern into a stage whose gate is otherwise complete (§14, 9/9 in
...
adapter (`SqliteStore`) are **deferred to Stage 2**. Stage 1 ships one adapter
each for those two seams.

The §5 rule itself is **not** repealed. Two adapters per seam remains the
standard; only the stage in which `GearSource` and `Store` meet it moves. The
`SimRunner` seam already meets it and is the worked example of what the other
two owe.

Stage 2 is not considered complete until both exist and are used.

## Consequences

- Stage 1's "runs deterministically offline from committed fixtures" claim
...
- Seam-shape risk is **deferred, not avoided**. If `GearSource` turns out to be
  ...
  Stage 2, and the fix then costs a change to both implementations at once
  rather than one.
- `MemoryStore` remains the only persistence in Stage 1, so nothing exercises
  serialization or partial-write behaviour. No Stage 1 result depends on
...
**Build `WclGearSource` in Stage 1.** Full compliance, but imports live-auth,
rate-limiting and point-budgeting work into a stage that is otherwise done, and
duplicates effort Stage 2 has already scoped.
...
one where being wrong costs the most. Worth revisiting if Stage 2 slips.
