...
>
> **Correction (Phase 1, `data/items/index.json` generation):** the two-character probe never happened to include an enchanted ring, which read as "finger" belonging on this list. It doesn't — TBC has "Enchant Ring - *" recipes (Enchanting profession), and the 25-combatant `slamaltman.raw.json` fixture shows finger slots enchanted in 14/50 cases, all resolving to real ring-enchant records in `db.json`. The non-enchantable set is **neck, waist, trinket** — verified two ways: `db.json`'s own `enchants[]` table has zero records targeting those three slots, and zero of those three slots are ever enchanted across all 25 combatants in the fixture. `packages/core/src/items.ts`'s `enchantable` field implements the corrected rule.
